...
fn parse(lines: &Vec<String>) -> Schedule {
    let mut sched = Schedule { timestamp: 0, bus: Vec::new() };

    sched.timestamp = lines.get(0).unwrap().parse().expect("invalid timeval");

    sched.bus = lines.get(1).unwrap().split(",").filter(|x| *x != "x")
        .map(|val| val.parse().expect("invalid bus id")).collect();

    sched
}

fn next_bus(sched: &Schedule) -> i64 {
    let wait_times : Vec<(i64, i64)> = sched.bus.iter().map(|x| (*x, x - sched.timestamp % x)).collect();
