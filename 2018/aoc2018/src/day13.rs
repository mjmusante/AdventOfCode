...
impl Track {
    ...
    pub fn new(input: &Vec<String>) -> Track {
        ...
        for l in input {
            grid.push(l.chars().map(|x| match x {
                'v' | '^' => '|',
                '<' | '>' => '-',
                _ => x
            }).collect());
    ...
    pub fn tick(&mut self) -> (Death, Death) {
        ...
        let mut first_dead = Death { is_dead: false, pos: (0, 0) };
        let mut last_dead = Death { is_dead: false, pos: (0, 0) };
        ...
        for c in &self.cart {
            if hm.contains_key(&c.pos) {
                if !first_dead.is_dead {
                    first_dead = Death { is_dead: true, pos: (c.pos.1, c.pos.0) };
                }
            ...
            let newc = Cart { pos: nextpos, dir: newdir, turn: newturn };
 
            if hm.contains_key(&nextpos) {
                if !first_dead.is_dead {
                    first_dead = Death { is_dead: true, pos: (nextpos.1, nextpos.0) };
                }
        ...
        if self.cart.len() == 1 {
            last_dead = Death { is_dead: true, pos: (self.cart[0].pos.1, self.cart[0].pos.0) };
        }
